    ...
    Eta(no="1",
        ...
        etas=[
            Eta.Time(
                ...
                is_arriving=True,
                is_scheduled=False,
                eta=_8601str(datetime.now(STZ)),
                eta_minute=datetime.now(STZ).minute,
            ),
            Eta.Time(
                destination="竹園邨",
                is_arriving=False,
                is_scheduled=True,
                eta=_8601str(datetime.now(STZ) + timedelta(minutes=8)),
                eta_minute=(datetime.now(STZ) + timedelta(minutes=8)).minute,
                remark="行車受阻@喇沙小學"
            ),
            Eta.Time(
                destination="竹園邨",
                is_arriving=False,
                is_scheduled=True,
                eta=_8601str(datetime.now(STZ) + timedelta(minutes=20)),
                eta_minute=(datetime.now(STZ) + timedelta(minutes=20)).minute,
            )
        ],
    ...
    Eta(no="4",
        ...
        etas=[
            ...
            Eta.Time(
                ...
                is_arriving=False,
                is_scheduled=True,
                eta=_8601str(datetime.now(STZ) + timedelta(minutes=62)),
                eta_minute=(datetime.now(STZ) + timedelta(minutes=62)).minute,
            ),
        ],
        timestamp=datetime.now().replace(tzinfo=STZ)
        ),
